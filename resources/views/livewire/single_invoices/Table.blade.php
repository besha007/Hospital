    ...
    <table class="table text-md-nowrap" id="example1" data-page-length="50"style="text-align: center">
        ...
        @foreach ($single_invoices as $single_invoice)
            <tr>
                ...
                <td>
                    <button title="تعديل" wire:click="edit({{ $single_invoice->id }})" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></button>
                    <button title="حذف" type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_invoice" wire:click="delete({{ $single_invoice->id }})" ><i class="fa fa-trash"></i></button>
                    <button title="طباعة" wire:click="print({{ $single_invoice->id }})" class="btn btn-info btn-sm"><i class="fas fa-print"></i></button>
                </td>
            </tr>
